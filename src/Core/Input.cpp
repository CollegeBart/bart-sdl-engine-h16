...
// TODO -- Not in use within IsKeyDown and IsKeyPressed
bool Input::ProcessKey(const SDL_Scancode & key, bool isReleased)
{
	bool isPressed = (isReleased && lastKeys[key]) && (!isReleased && keys[key]);
	if (isPressed)
	{
		lastKeys[key] = keys[key];
	}
